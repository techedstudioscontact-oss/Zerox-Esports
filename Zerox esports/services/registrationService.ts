...
import { collection, addDoc, query, where, getDocs, doc, runTransaction, serverTimestamp, orderBy, onSnapshot } from 'firebase/firestore';
import { Registration, User } from '../types';

// Real-time subscription — fires whenever anyone joins or leaves the tournament
export const subscribeToTournamentRegistrations = (
    tournamentId: string,
    ...
    const unsub = onSnapshot(q, (snap) => {
        const regs = snap.docs.map(d => d.data() as Registration);
        regs.sort((a, b) => a.registeredAt - b.registeredAt);
        callback(regs);
    }, (err) => {
        console.warn('registrations listener:', err.code);
        callback([]);
    });
    return unsub;
};
