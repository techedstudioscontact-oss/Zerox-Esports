...
export function CommentSection({ contentId, user }: CommentSectionProps) {
    ...
    useEffect(() => {
        if (!contentId) return;
        // Comments require authentication — don't start listener if not logged in
        if (!user) {
            setComments([]);
            setLoading(false);
            return;
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            console.warn("Comments listener closed:", error.code);
            setLoading(false);
        });

        return () => unsubscribe();
    }, [contentId, user]); // Re-run when user logs in/out
